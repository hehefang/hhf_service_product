...
import com.hhf.constants.product.ProductConstants.BrandShowDetail$Status;
...
import com.hhf.product.dao.BrandShowDetailMapper;
import com.hhf.product.dao.BrandShowMapper;
...
public class BrandShowServiceImpl implements IBrandShowService {
		...
		if (result > 0) {
			...
			for (BrandShowDetail detail : details) {
				...
				if (detail.getbSDId() != null && detail.getbSDId() != 0) {
					...
					Set<String> keys = this.redisTemplate.keys("bsdpage"+brandShowId+"p*");
					if(keys!=null&&keys.size()>0){
						this.redisTemplate.delete(keys);
					}
					keys=this.redisTemplate.keys("bsdprods"+brandShowId+"p"+detail.getProdId());
					if(keys!=null&&keys.size()>0){
						this.redisTemplate.delete(keys);
					}
					this.redisTemplate.delete(brandShowId+"p"+detail.getbSDId());	
					this.redisTemplate.delete("lowestp"+brandShowId);
				} else {
	...
	public int startBrandShow(int brandShowId) {
        ...
        this.redisTemplate.delete("bsonline");
